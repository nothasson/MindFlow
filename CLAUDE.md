...
- **个人产品 / 本地优先** — MindFlow 首先是面向个人学习者的产品，不以多人协作、社交传播、中心化平台为默认目标。新增能力优先采用单用户、本地优先（local-first）方案，尽量不依赖持续在线的远程服务器；能用本地文件、本地数据库、浏览器能力或单机部署解决的问题，不要先设计成云端服务。
- **全程使用中文回复** — 与用户的所有对话、代码注释、commit 信息、文档内容、review 输出等一律使用中文。禁止在对话中输出英文段落或英文表格。代码中的变量名、函数名等标识符保持英文，但注释和说明必须中文。
...
- **不主动引入多人/社交化能力**，除非明确证明对个人学习闭环有必要
- **尽量不依赖远程服务器**，优先本地文件、浏览器本地能力、本地数据库或单机 Docker 部署
- **分享/迁移能力优先做导出导入或静态产物**，不要默认设计成依赖中心化在线服务
- **新增功能若必须依赖远程服务**，先说明为什么本地方案不可行，再引入可选服务器能力
