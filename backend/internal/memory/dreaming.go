...
func (d *DreamingSweep) Run(ctx context.Context, date string) error {
	...
	if !datePattern.MatchString(date) {
		return fmt.Errorf("日期格式无效: %s（需要 YYYY-MM-DD）", date)
	}
	...
	// 防御性检查：LLM 返回空内容时不覆盖现有记忆
	if strings.TrimSpace(updatedMemory) == "" {
		return fmt.Errorf("LLM 返回空内容，跳过写入以保护现有记忆")
	}
	...
	if err := d.store.SaveLearnings(date, content); err != nil {
